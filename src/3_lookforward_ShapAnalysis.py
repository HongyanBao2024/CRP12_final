...
import numpy as np
import itertools
from neuralforecast.models import RNN, DeepAR, DeepNPTS
...
shap_results_group = pd.DataFrame(columns=['unique_id', 'ds','y', 'temperature_ex', 'promo_ex','DeepNPTS','DeepNPTS_Base','temperature_ex_shap','promo_ex_shap'])
...
for unique_id in df_demand_long['unique_id'].unique().tolist():
    ...
    for i in list(range(num_test)):
        ...
        shap_results_eachdate['DeepNPTS'] = pred['DeepNPTS'].astype(float).round(2).values
        future_df_grouped['DeepNPTS'] = [pred['DeepNPTS'].astype(float).round(2).values.tolist()]
        future_df_grouped['DeepNPTS_pred_lag2'] = future_df_grouped['DeepNPTS'][0][2]
        ...
        shap_results_eachdate['DeepNPTS_Base'] = pred['DeepNPTS'].astype(float).round(2).values
        future_df_grouped['DeepNPTS_Base'] = [pred['DeepNPTS'].astype(float).round(2).values.tolist()]
        ...
        for feature in ['temperature_ex', 'promo_ex']:
            ...
            for subset in [[], ['temperature_ex'], ['promo_ex']]:
                ...
                fut_S = modify_future(future_df, subset,ref_temperature,ref_promo)
                fut_S_i = modify_future(future_df, subset_with_i,ref_temperature,ref_promo)
                ...
                marginal_contrib = (pred_S_i['DeepNPTS'].astype(float).round(2) - pred_S['DeepNPTS'].astype(float).round(2))
                weight = 0.5
    ...
    shap_results_group_filter = shap_results_group.loc[shap_results_group['unique_id'] == unique_id, ['y_lag2','DeepNPTS_pred_lag2']]
    Accuracy_adjusted_MAPE = Accuracy_adjusted_MAPE_TCCC(shap_results_group_filter['y_lag2'] , shap_results_group_filter['DeepNPTS_pred_lag2'])
...
Accuracy_adjusted_MAPE = Accuracy_adjusted_MAPE_TCCC(shap_results_group['y_lag2'] , shap_results_group['DeepNPTS_pred_lag2'])
